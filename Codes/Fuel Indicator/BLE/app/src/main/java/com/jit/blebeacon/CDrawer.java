...

{
		...
		if (isCreated)
		{
			if (mDrawThread.GetDead2().booleanValue())
			{
				...
				if ((!paramBoolean.booleanValue())
						|| (!mDrawThread.GetDead().booleanValue()))
	...
	class CDrawThread extends Thread
	{
		...
		public void doDraw(Canvas paramCanvas)
		{
			...
			int BuffIndex = Math.abs(mBuffer.length / 2
					- paramCanvas.getWidth()) / 2;
			...
			while (StratX < width - 1)
			{
				try
				{
					...
					try
					{
						StartBaseY = mBuffer[(mBuffIndex - 1)] / scale;
					}
					catch (Exception ex)
					{
						StartBaseY = mBuffer[0];
					}

					int StopBaseY = mBuffer[mBuffer.length - 1] / scale;
					/*
					 * if (StartBaseY > height / 2) { StartBaseY = 2 + height /
					 * 2; int checkSize = height / 2; if (StopBaseY <=
					 * checkSize) return; StopBaseY = 2 + height / 2; }
					 */

					int StartY = StartBaseY + height / 2;
					int StopY = StopBaseY + height / 2;
					paramCanvas.drawLine(StratX, StartY, StratX + 4, StopY,
							mLinePaint);
					cu1++;
					mBuffIndex++;
					StratX += 4;
			...
			paramCanvas.drawText("Notification\n" + CurrentMessage, 20, 200,
					mRedPaint);
		}
